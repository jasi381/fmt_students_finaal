...
import 'package:students/presentation/screens/classes/home_classes/view_schedule.dart';
...
class HomeTeacherDetails extends StatefulWidget {
  ...
  @override
  State<HomeTeacherDetails> createState() => HomeTeacherDetailsState();
}

class HomeTeacherDetailsState extends State<HomeTeacherDetails> {
  ...
  Future<void> _checkAndProceed(VoidCallback action) async {
    final sessionPriceString = Provider.of<TeacherDetailsProvider>(context, listen: false).teacherDetails?.homePrice ?? "0";
    ...
    if (isSufficient) {
      ...
      // Show dialog if balance is insufficient
      showDialog(
        context: context,
      ...
      bottomNavigationBar: BottomAppBar(
        child: Container(
          ...
          child: ElevatedButton(
            ...
            onPressed: () {
              _checkAndProceed((){
                push(context, ViewScheduleScreen(teacherId: widget.teacherId, teacherName: widget.teacherName,sessionPrice:widget.sessionPrice,apiType: widget.apiType,));
              });
            },
            child: const PoppinsText(text: "Book a Session",textColor: Colors.white,fontSize: 18,),
          ),
        ),
      ),
    );
